...
###############
##
## xeno-canto: Train and validation data
##
###############
...
# # Save the xenocanto object as a LaTeX table
# xenocanto %>% 
#   slice_head(n = 15) %>% 
...
#          floating = TRUE,
#          label = "tab:xenocanto") %>% 
#   print(file = here(tables_dir, "xenocanto.tex"))
...
###############
##
## warblrb10k: Train and validation data
##
###############
...
# # Save the warblrb10k object as a LaTeX table
# warblrb10k %>% 
#   slice_head(n = 15) %>% 
#   xtable(sanitize.colnames.function = bold,
#          booktabs = TRUE,
#          floating = TRUE,
#          label = "tab:warblrb10k") %>% 
#   print(file = here(tables_dir, "warblrb10k.tex"))
...
# # (Run only when the external hard drive is connected)
# bats <- list.files(
#   path = "/media/esther/TFM_Esther",
#   pattern = "[wW][aA][vV]$",
#   full.names = TRUE,
#   recursive = TRUE
# ) %>%
#   data.frame(path_wav = .) %>%
#   mutate(
#     zone = str_extract(path_wav, pattern = "(?<=TFM_Esther/)\\w*"),
#     territory = str_extract(path_wav, pattern = "(?<=TFM_Esther/).*/(.*)/(?<!Data/)") %>%
#       str_extract(pattern = "(?<=/)\\w*"),
#     town = str_extract(path_wav, pattern = "(?<=TFM_Esther/).*/(.*)/(?<!Data/)") %>%
#       str_extract(pattern = "(?<=/)\\w*(?=/$)"),
#     filename_wav = str_extract(path_wav, pattern = "[:alpha:]{3}-\\w*.wav$")
#   )
# 
# # Select audio samples from unlabelled wavs paths
# set.seed(999)
# mcng_wav_sub <- sample(x = bats$path_wav, size = 500)
# 
# # Copy the sample files to local path
# file.copy(from = mcng_wav_sub, to = bats_wav_dir)
# 
# # Get list .wav files local path 
# test1 <- list.files(
#   path = xevi_roger_dir,
#   pattern = "[wW][aA][[vV]",
#   full.names = TRUE
...
mcng <- list.files(
  ...
  mutate(
    ...
    # filename_wav_new = str_extract(path_wav, pattern = "(?<=TFM_Esther/).*/(.*)") %>%
      # gsub("(/|-)", "_", .),
    # path_wav_new = str_extract(path_wav, pattern = ".*/")
  )

# Copy the sample files to local path
# file.copy(from = mcng$path_wav, to = test_dir)
